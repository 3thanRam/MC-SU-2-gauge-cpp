...
def Graph4stuff(ax, ax2, ax3, N, Imax, Beta_array, Ldata):
    ...
    ax.plot(Xspace1, Xspace1/4, linestyle='--',
            label='1*1 Strong coupling lim', color='red')
    ax.plot(Xspace1, (Xspace1/4)**4, linestyle='--',
            label='2*2 Strong coupling lim', color='green')
    Weakc = 1-0.75/Xspace2
    ax.plot(Xspace2, Weakc, linestyle='--',
            label='weak coupling lim', color='grey')
...
def graph5(ax, N, Beta_array, Ldata):
    ...
    for b, beta in enumerate(Betalist):
        ...
        ax.plot(size_array[mask], Y[mask],
                label="B="+str(beta), color=COLORS[b])
    np.seterr(over='warn')
...
def Graph6(ax, N, Imax, Beta_array, Ldata):
    ...
    ax.plot(Beta_spaceS[Smask], Strlim[Smask],
            label='Strong coupling lim: $-ln\\;\\frac{\\beta}{4}$', color='red')

    Weaklim = Weakcoupl(Beta_spaceW)
    Wmask = (Weaklim > 0.01) & (Weaklim < 10)
    ax.plot(Beta_spaceW[Wmask], Weaklim[Wmask],
            label='Weak coupling lim: $\\exp{\\frac{-6\\pi^{2}}{11}(\\beta-2)}$', color='blue')
...
def DRAW(Graphnumb):
    Nlist = []
    location = path.removesuffix(
        '/plotting')+"/graphdata/json_data" + str(Graphnumb) + ".json"
    ...
    for p in range(numbplots):
        # skip data that is empty or all zeros
        if Fullplotdata[p] == [] or not VALS[p][:, 1].any():
            continue
        ...
        if Graphnumb != 3:
            ax.plot(xdata, ydata, color=color, marker=marker,
                    ms=4, linestyle=linestyle, label=graphinfo)
        else:
            Err = np.zeros((2, len(xdata)))
            Err[0] = VALS[p][:, 2]  # ydata[:]-VALS[p][:, 2]
            Err[1] = VALS[p][:, 3]  # VALS[p][:, 3]-ydata[:]
            ax.errorbar(xdata, ydata, yerr=Err, color=color, fmt='o',
                        ms=4, linestyle=linestyle, label=graphinfo)
    ...
    fig.tight_layout()
    plt.savefig(path.removesuffix('/plotting') +
                "/graphdata/figure"+str(Graphnumb)+".pdf")
    plt.show()


def polyfct(x, *params):
    return sum([p*x**i for i, p in enumerate(params)])


def Otime():
    location = path.removesuffix(
        '/plotting')+"/graphdata/json_datatime.json"
    with open(location, "r") as f:
        data_loaded = json.load(f)
        Lsizes, Ltimes = data_loaded["Lsizes"], data_loaded["Ltimes"]
    fig = plt.figure()
    ax = plt.subplot(111)
    ax.plot(Lsizes, Ltimes, marker='o', ms=4, linestyle='-',
            label='Time to bring system to equilibrium')

    size_array = np.linspace(0, max(Lsizes), 10**2)

    for c in range(1, 6):
        def fct(x, param): return param*x**c
        param = curve_fit(fct, Lsizes, Ltimes,
                          p0=0)[0]
        Y = fct(size_array, param)
        ax.plot(size_array, Y, "--", label='O('+str(c)+') Fit ')
    Grsetup(ax, 'Lattice size', 'Time (s)',
            'Time to bring system to equilibrium', 0, max(Ltimes))
    plt.savefig(path.removesuffix('/plotting') +
                "/graphdata/timefigure.pdf")
    plt.show()
